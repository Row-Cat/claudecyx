...
def test_evaluate_returns_new_state_object_not_mutated_input():
    original = {}
    _alerts, new_state = evaluate_window(
        "five_hour", 0.5, None, original, 0.90, 0.95, 1.0, "org"
    )
...
def test_warning_fires_once_across_polls():
    ...
    alerts, state = evaluate_window(
        "five_hour", 0.92, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert len(alerts) == 1
    ...
    alerts, state = evaluate_window(
        "five_hour", 0.93, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert alerts == []
    assert state.get("alerted_warning") is True
    assert not state.get("alerted_critical")
...
def test_critical_fires_once_across_polls():
    state = {}
    alerts, state = evaluate_window(
        "five_hour", 0.96, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert len(alerts) == 1
    ...
    alerts, state = evaluate_window(
        "five_hour", 0.97, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert alerts == []
...
def test_resets_at_none_does_not_trigger_reset_alert():
    state = {}
    alerts, state = evaluate_window(
        "five_hour", 0.5, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert alerts == []
...
def test_at_limit_fires_max_priority_alert():
    state = {}
    alerts, state = evaluate_window(
        "five_hour", 1.0, None, state, 0.90, 0.95, 1.0, "org"
    )
    assert len(alerts) == 1
    assert alerts[0].kind == AlertKind.CRITICAL
